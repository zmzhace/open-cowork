...
# Gather manual tool schemas
MANUAL_TOOL_SCHEMAS = [t.TOOL_SCHEMA for t in TOOLS.values()]
...
async def execute_tool_async(tool_name: str, tool_input: dict) -> dict:
    """异步执行工具（使用 asyncio.to_thread / MCP 调用）"""
    # Check if this tool is handled by our manual python modules
    if tool_name in TOOLS:
        tool_module = TOOLS[tool_name]
        try:
            result = await asyncio.to_thread(tool_module.execute, **tool_input)
            return result
        except Exception as e:
            import traceback
            logger.error(f"Manual Tool {tool_name} failed: {e}")
            return {
                "error": f"Tool execution failed: {str(e)}",
                "traceback": traceback.format_exc()
            }
            
    # If not a manual tool, dispatch to Windows-MCP
...
你同时接入了 **原生 MCP 工具 (Windows-MCP)** 和 **视觉后备工具 (screen 等)**。

### 1️⃣ 首选：Windows-MCP 工具群 (如可用，务必优先使用)
如果任务是操作标准 Windows 应用程序，请**优先选择 MCP 暴露给你的原生工具 (如 App, Window, Click, Type, Snapshot 等)**。
- 优点：通过 Win32 API 交互，**速度极快，无需截图定位坐标**。
- 操作逻辑：可以利用 MCP 抓取 UI 树/进程列表，并直接用对应的 MCP 工具与组件交互。

### 2️⃣ 兜底：视觉截图+模拟工具群
当你面对非标准 GUI (如某些游戏、用特殊引擎绘制界面的 APP，或是 MCP 无法识别内部元素的复杂网页/界面时)，请降级使用底层的 `screen` 截图并结合视觉模型点击/输入。
- **screen** — 截屏、鼠标点击、键盘输入、等待
- **window** — 列出/聚焦桌面窗口
- **process** — 查询/结束进程
- **exec** — 运行系统命令 (`start 程序名` 可以秒开程序)
- **file/clipboard** — 文件与剪贴板读写

**智能选择原则**：你是一个全能 Agent，你应当通过自己的理解，决定哪个工具对于当前的情况最快最准。如果 MCP 工具报告 `Element not found` 或没能按预期工作，**不要原地重试**，请立刻回退到大杀器：截图 `screen(screenshot)` 后通过视觉找坐标！
...
async def run_agent(
    ...
    # Lazy init MCP connection once
    if not mcp_client.session:
        try:
            logger.info("Initializing Windows-MCP connection...")
            await mcp_client.connect()
        except Exception as e:
            logger.error(f"Failed to connect to Windows-MCP: {e}")
            if on_step:
                on_step(0, "Warning: Windows-MCP offline. Falling back to visual tools only.")
                
    # Build complete Tool Schemas payload
    active_tools = list(MANUAL_TOOL_SCHEMAS)
    if mcp_client.session:
