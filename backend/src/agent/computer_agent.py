...
### 方式 1：命令行直接启动（最快！推荐！）
```
exec(command="start qq")       — 打开 QQ
exec(command="start WeChat")   — 打开微信
exec(command="start notepad")  — 打开记事本
exec(command="start chrome https://baidu.com")  — 打开Chrome并访问百度
```
如果不确定程序名，先用 `app_finder` 查找。
